...
const CROP_ARRAY = [
  {
    ...
    name: "carrot",
    image:
      "https://ipfs.io/ipfs/bafkreib6uhqu4h22ii2b2mlk3qetekjs27w4pyoaqxcuqgkcl2yjkjuvb4",
  },
  {
    id: 1,
    name: "potato",
    image:
      "https://ipfs.io/ipfs/bafkreia773gxzvwioptbs7paxcstho2amkmmanj2fzexitxhmokamy43fu",
  },
  {
    id: 2,
    name: "corn",
    image:
      "https://ipfs.io/ipfs/bafybeidhfgjqtinwfbqd267tntjmmzkrngkr2jolvbzksh7kayxr7ndeom",
  },
  {
    id: 3,
    name: "golden-carrot",
    image:
      "https://ipfs.io/ipfs/bafkreiecspw7et4pof4iadgmuvwrmsqi3bcplu5rmnihpr7yxscf7se2oi",
  },
  {
    id: 4,
    name: "sprouting-potato",
    image:
      "https://ipfs.io/ipfs/bafkreifo3gn27ddsbviumbywxhlb5qhb3b7v3tu3cl5ttt32nldmwrkhci",
  },
  {
    id: 5,
    name: "rainbow-corn",
    image:
      "https://ipfs.io/ipfs/bafybeigqufvvagqba2d4a7qotqlyz4qcef3rjhyoo2ztyet5wivvkr5qpq",
  },
];
...
function App() {
  ...
  const loadBlockchainData = async () => {
    ...
    const crops = new ethers.Contract(
      Crops.networks[networkId].address,
      Crops.abi,
      signer
    );
    ...
    const nftFarm = new ethers.Contract(
      NFTFarm.networks[networkId].address,
      NFTFarm.abi,
      signer
    );
    setNFTFarm(nftFarm);
    ...
    const pointsBalance =
      +ethers.utils.formatUnits(pointsDebt, "ether") +
      +(futureTime(0) - +userInfo.lastUpdateAt) *
        +emissionRate *
        +ethers.utils.formatUnits(stakedAmount, "ether");
    ...
    let string = "";
    collectedCrops.forEach((value, index, array) =>
      value > 0
        ? (string =
            string +
            `${value} ${CROP_ARRAY[index].name}${value > 1 ? "s" : ""}${
  ...
  const claimNFTs = async (quantities) => {
    ...
    quantities.forEach((i, index) =>
      i > 0 ? selectedCrops.push(index) : null
    );
      ...
      {loading ? (
        ...
        <div className="container-fluid mt-5">
            ...
            {listedCrops.map((crop, key) => {
              ...
              return (
                <div key={key} className="col-md-2 mb-2">
                  <img
                    src={CROP_ARRAY[crop.id.toNumber()].image}
                    alt=""
                    className="mw-100"
                  />
                  <div>
                    Price: {ethers.utils.formatUnits(crop.price, "ether")}
                  </div>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      quantities[key] = event.target.value
                        ? event.target.value
                        : 0;
                      ...
                      setTotalCost(
                        quantities.reduce(
                          (prev, i, index) =>
                            +prev.toString() +
                            +(
                              +i * +listedCrops[index].price.toString()
                            ).toString(),
                          0
                        )
                      );
          ...
          <div className="row">
            <main
              role="main"
              className="col-lg-12 justify-content-center d-flex mx-auto text-center"
            >
